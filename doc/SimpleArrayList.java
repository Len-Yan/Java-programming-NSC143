 ...
 * updated 5/10/06, 5/7/08, 5/11/08, 11/4/09, 2/9/12, 10/27/14 by vo
 */
public class SimpleArrayList<E> implements List<E> {
   ...
   */
  @SuppressWarnings("unchecked")  // Comment this out and compile to see the warning.
  public SimpleArrayList( int initialCapacity ) {
    items = (E[]) new Object[initialCapacity];  // Shows "unchecked cast" warning if not suppressed.
    numItems = 0;
  }
  ...
  private void ensureCapacity( int minCapacity ) {
    ...
    E[] newList = (E[]) new Object[newCapacity];  // Shows "unchecked cast" warning if not suppressed.
    for ( int i = 0; i < numItems; i++ ) {
      newList[i] = items[i];
    }
    items = newList;
   ...
   * out of range ( less than 0 or greater than size( ) )
   */
   ...
   * @throws IndexOutOfBoundsException if the position is 
   * out of range ( less than 0 or greater than or equal to size( ) )
   */
  ...
  class SimpleArrayListIterator<E> implements Iterator<E> {
     ...
     * @param theList the list served by the iterator
     */
