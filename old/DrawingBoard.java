...
public class DrawingBoard{
  ...
  public void passxy(int x, int y){
    ...
    while(itr.hasNext()){                                    //iterator to find pass point x y
      Shape temp = itr.next();
      boolean testp = temp.isOn(x,y);
      System.out.println("tya2");
      
      if (testp == true){
        Iterator<Shape> itr2 = list.iterator();              //set all not selected after find shape fit x y
        while(itr2.hasNext()){
          Shape temp2 = itr.next();
          temp2.setSelected(false);
        }
        
        list.add(temp);                               // move find shpae to last
        list.get(list.size()-1).setSelected(true);    // set true
        list.remove(count);
        break;
      }
      count++;
  ...
  public void changeC(Color c){
    ...
    if (list.get(list.size()-1).isSelected() == true){
